/* ===== MOTION (Missão 12 · Sprint 1 · 1C) — porto para React (M25 · Fase 17) =====
   ...
   em runtime e repõe os transforms.

   Porto verbatim de legacy/js/motion.js, com UMA diferença de forma: no Vanilla
   o HTML era estático e `.aurora` existia no load; aqui o palco React monta
   depois — por isso a aurora é procurada LAZY dentro do handler, não capturada
   no arranque. Corre 1× (importado por main.jsx antes do 1º render) e expõe
   window.Motion, exatamente como o script clássico. */
...
(function () {
  ...
  const aur = () => document.querySelector('.aurora'); // lazy: o palco React monta depois
  ...
  const sAur = new M.Spring(2, M.TOK.slow, x => {
    const a = aur(); if (a) a.style.transform = 'translate(' + x[0].toFixed(0) + 'px,' + x[1].toFixed(0) + 'px)';
  });
  function reset() {
    if (panel) { panel.style.transform = ''; panel = null; }
    if (btn) { btn.style.transform = ''; btn = null; }
    const a = aur(); if (a) a.style.transform = '';
