...
function renderCalendar(){
  ...
  for(let d=1;d<=ndays;d++){const ds=calY+'-'+String(calM+1).padStart(2,'0')+'-'+String(d).padStart(2,'0');
    ...
    cells+=`<div class="cal-cell ${ds===tT?'today':''}${ds<tT&&okD.has(ds)?' past-ok':''}" onclick="pickDay('${ds}')" title="${escHTML(evs.map(e=>e.title).join(', '))}"><div class="cal-num">${d}</div><div class="cal-dots">${dots}</div>${evs.length?`<div class="cal-ev">${escHTML(evs[0].title)}</div>`:''}</div>`;}
  ...
  const upH=up.length?up.map(e=>{const t=EVT[e.type]||EVT.outro,dd=daysUntil(e.date);
    return `<div class="up-item"><span class="up-dot" style="background:${t.c}"></span><span class="up-d">${e.date.slice(8,10)}/${e.date.slice(5,7)}</span><span class="up-t">${escHTML(e.title)}</span><span class="up-x" style="color:${t.c}">${dd===0?'hoje':dd===1?'amanhã':'em '+dd+'d'}</span><span class="up-del" onclick="delEvent('${e.id}')">✕</span></div>`;}).join(''):'<div class="up-empty">Sem prazos próximos.</div>';
...
function renderDeadlineBanner(){
  ...
  const it=all.slice(0,3).map(x=>(x.k==='obj'?'🎯 ':'')+escHTML(x.t)+' ('+lbl(x)+')').join(' · ');
