...
async function esperar(s, expr, ms = 8000) {
...
const texto = (s, sel) => s.avaliar(`(document.querySelector(${JSON.stringify(sel)})||{}).innerText||''`);
const clicar = (s, sel) => s.avaliar(`(()=>{const el=document.querySelector(${JSON.stringify(sel)});if(!el)return false;el.click();return true})()`);
const clicarTexto = (s, dentro, t) => s.avaliar(`(()=>{const el=[...document.querySelectorAll(${JSON.stringify(dentro + ' button')})].find(b=>b.textContent.trim()===${JSON.stringify(t)});if(!el)return false;el.click();return true})()`);
...
async function abrir(chrome, site, nome, cfg, opcoes = {}) {
