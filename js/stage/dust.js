...
uniform float uTime;uniform vec2 uRes;uniform float uFall;uniform float uPix;
varying float vA;varying float vTint;
void main(){
  float wrap=uRes.y+8.;
  float vy=mix(-aVy,aVy*.7,uFall); /* sobe por defeito; à chuva desce devagar */
  float y=mod(position.y+vy*uTime+4.,wrap)-4.;
  float x=mod(position.x+aVx*uTime+sin(uTime*.42+aPhase)*3.6*position.z+4.,uRes.x+8.)-4.;
