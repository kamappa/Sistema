...
function favicon(u){const s=urlSegura(u);return s?'https://www.google.com/s2/favicons?sz=64&domain='+encodeURIComponent(new URL(s).hostname):''}
/* o texto do Radar e do relatório vem do modelo (e da web): entra sempre escapado — ver escHTML em engine.js */
const idSeguro=id=>/^[0-9a-f-]{8,64}$/i.test(String(id||''));
function renderNews(){
  ...
  el.innerHTML=Object.keys(byD).sort().reverse().map(d=>{
    ...
    return `<div class="up-lbl">${escHTML(lbl)}</div>`+byD[d].map(i=>{const a=RAREA[i.area]||RAREA.ai;const ic=favicon(i.url);const u=urlSegura(i.url);
      ...
      return `<div class="rd-item ${i.impact==='alto'?'rd-hot':''} ${i.area==='vaga'?'rd-vaga':''}">
        ${ic?`<img class="rd-ico" src="${escHTML(ic)}" alt="" loading="lazy" onerror="this.style.display='none'">`:''}
        <div class="rd-b">
          ${i.impact==='alto'?'<div class="rd-hotlbl">⚡ ALTO IMPACTO</div>':''}
          ${u?`<a class="rd-t" href="${escHTML(u)}" target="_blank" rel="noopener">${escHTML(i.title)}</a>`:`<span class="rd-t">${escHTML(i.title)}</span>`}${isNew?'<span class="rd-new">NOVA</span>':''}
          ${i.summary?`<div class="rd-s">${escHTML(i.summary)}</div>`:''}
          ${i.relevance?`<div class="rd-r">→ ${escHTML(i.relevance)}</div>`:''}
          <div class="rd-src">${escHTML(i.source)} <span class="wchip" style="border-color:${a.c};color:${a.c};padding:1px 7px;font-size:9px;margin-left:6px">${a.l}</span>${tm?` <span class="rd-time">· ${tm}</span>`:''}</div>
          ${(i.missao&&i.missao.t&&idSeguro(i.id))?`<button class="mini warm" style="margin-top:7px" onclick="acceptRadarMission('${i.id}',event)">${S.radarAccepted[i.id]?'✓ Missão aceite':'＋ Aceitar missão do Radar'}</button>`:''}
        </div></div>`}).join('')}).join('');
...
function renderOracleRep(){
  ...
  const prop=(r.propostas||[]).map(p=>`<div class="rd-item"><span style="flex:none">🜂</span><div class="rd-b"><div class="rd-t">${escHTML(p.t)}</div><div class="rd-s">${escHTML(p.why)}</div></div></div>`).join('');
  const mp=(r.missoes_propostas||[]).map((m,i)=>`<div class="rd-item"><span style="flex:none">⚔️</span><div class="rd-b"><div class="rd-t">${escHTML(m.t)}</div><div class="rd-s">${escHTML(m.why)}</div></div><button class="mini" onclick="acceptOracleMission(${i},event)">＋ Aceitar</button></div>`).join('');
  const rec=(r.recursos||[]).filter(x=>x&&urlSegura(x.url)).map(x=>`<div class="rd-item"><span style="flex:none">📖</span><div class="rd-b"><a class="rd-t" href="${escHTML(urlSegura(x.url))}" target="_blank" rel="noopener">${escHTML(x.titulo||urlSegura(x.url))}</a><div class="rd-s">${escHTML(x.porque)}</div><div class="rd-src">${escHTML(x.fonte)}</div></div></div>`).join('');
  el.innerHTML=`
    <div class="tr-stats"><span class="wchip gold">📜 Relatório de ${escHTML(dt.slice(8,10))}/${escHTML(dt.slice(5,7))}</span></div>
    ${r.resumo?`<div class="orc-sec">${escHTML(r.resumo)}</div>`:''}
    ${r.treino?`<div class="orc-sec"><b>🏋️ Treino:</b> ${escHTML(r.treino)}</div>`:''}
    ${r.sono?`<div class="orc-sec"><b>😴 Sono:</b> ${escHTML(r.sono)}</div>`:''}
    ${r.estudo?`<div class="orc-sec"><b>📚 Estudo (vault):</b> ${escHTML(r.estudo)}</div>`:''}
    ${r.alerta&&r.alerta!=='null'?`<div class="orc-sec" style="color:#fca5a5"><b>⚠ Alerta:</b> ${escHTML(r.alerta)}</div>`:''}
    ...
    ${r.efemeride&&r.efemeride!=='null'?`<div class="orc-sec"><b>🕯 Efeméride:</b> ${escHTML(r.efemeride)}</div>`:''}
    ${r.profecia&&r.profecia!=='null'?`<div class="orc-sec orc-prof"><b>🔮 Profecia:</b> ${escHTML(r.profecia)}</div>`:''}
    ${r.recompensa?`<div class="orc-sec"><b>🎁 Recompensa sugerida:</b> ${escHTML(r.recompensa)}</div>`:''}
    ${r.titulo?`<div class="orc-sec"><b>🎖 Título da semana:</b> ${escHTML(r.titulo)}</div>`:''}
    ${r.legado?`<div class="orc-leg">👑 ${escHTML(r.legado)}</div>`:''}`;
